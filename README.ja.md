...
ボードに関するレスポンスはすべて開示ポリシーを通る（[ADR-012](docs/adr-012-metadata-minimization.md)）。
content-blind モードでは opaque ref・構造化コード・件数だけになり、ホスト名・絶対パス・自由文は返らない。
運用行は文書化された期間で失効する（`python -m app.retention`）。台帳は失効しない。
...
(cd backend && alembic upgrade head)    # migration 012 まで適用・"self" Actor を seed
```
...
- 🚧 **Tests**: SQLite 396 ケース（台帳の不変条件・並行書き込み安全性・run-state 投影の idempotency・調整レイヤー・パス重なり判定・git 資源 claim・MCP tool surface）に加え、実マイグレーションを適用し実コネクションで承認台帳と資源 claim を競合させる Postgres スキーマ整合 36 ケース。両方 CI で実行（Postgres ジョブは `postgres:16` サービス）。ローカルでは `AXONRELAY_TEST_POSTGRES_URL=... pytest tests/test_postgres_schema.py`。
