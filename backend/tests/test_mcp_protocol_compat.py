...
def test_a_failed_delivery_is_reported_rather_than_retried(waiting_task, mcp_db, monkeypatch):
    """Nothing re-sends a decision the graph did not confirm.

    A second delivery answers whichever interrupt the graph has reached by
    then, and a timeout after Platform accepted the resume is indistinguishable
    from one it never received - so an automatic retry would be guessing about
    exactly that. The decision stands in the ledger and the operator is told,
    ...
    assert "Nothing re-sends it" in message
    assert "reject_task" in message, "the operator needs the way through"

    assert len(attempts) == 1, "one attempt, and no automatic second"
    assert len(crud.get_approvals(mcp_db, waiting_task.id)) == 1, "the decision stands"
