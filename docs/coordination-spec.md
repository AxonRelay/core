...
さらなる限界: ラッパは `command git` や絶対パス指定で迂回できる。前置き（`gitsafe git …`）かシェル関数で `git` を覆う運用が前提であり、PATH 上に `git` という名前のラッパを置く方式は採らない（[ADR-007](./adr-007-gitsafe-enforcement-path.md)）。同 ADR が引用する隣接プロジェクトの `gh` ラッパで実測された制約と、同じ性質のものである。
