...
さらなる限界: ラッパは `command git` や絶対パス指定で迂回できる。前置き（`gitsafe git …`）かシェル関数で `git` を覆う運用が前提であり、PATH 上に `git` という名前のラッパを置く方式は採らない（[ADR-007](./adr-007-gitsafe-enforcement-path.md)）。保護対象より強制経路の影響範囲が広くなるため、明示 opt-in を維持する。
